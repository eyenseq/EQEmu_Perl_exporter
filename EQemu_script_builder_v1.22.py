...
def render_plugin_template(template: str, params: Dict[str, Any]) -> str:
    ...
    def repl(m: re.Match) -> str:
        ...
        if key not in params:
            ...
            raise ValueError(f"Missing template param: '{key}'. Provided keys: [{avail}]")
...
def validate_blocks(blocks, plugin_registry) -> list[ValidationIssue]:
    ...
    # -------------------------
    # 1.5) For inifinate loop helpers
    # -------------------------
    def _int_or_none(x):
        if isinstance(x, int):
            return x
    ...
    def _step_or_none(inc: str):
        inc = (inc or "").strip()
        if inc == "++":
            return 1
        if inc == "--":
            return -1
        m = re.match(r'^\+=\s*([0-9_]+)\s*$', inc)
        if m:
    ...
    # -------------------------
    # 2) Duplicate EVENT_* blocks + hierarchy safety
    # -------------------------
    counts: Dict[str, int] = {}
    nested_events: List[Block] = []

    def _walk_with_parent(bs, parent=None):
        for b in bs:
            yield b, parent
            for c in getattr(b, "children", []) or []:
                yield from _walk_with_parent([c], b)

    for b, parent in _walk_with_parent(blocks, parent=None):
        ...
        # EVENT blocks should only exist at root/top-level
        if parent is not None:
            nested_events.append(b)

    for ev, n in counts.items():
        if n > 1:
            issues.append(ValidationIssue(
                "warn",
                f"Duplicate handler: {ev} appears {n} times. (You usually want only one.)",
                ev
            ))

    for b in nested_events:
        issues.append(ValidationIssue(
            "error",
            "EVENT blocks must be top-level (not nested). Drag it back to the root.",
            b.label,
            block_ref=b
        ))
    ...
    for b in _walk_blocks(blocks):
        ...
        try:
            ...
            issues.append(ValidationIssue(
                "error",
                f"Plugin '{pid}' template render failed: {type(e).__name__}: {e}",
                b.label,
                block_ref=b
            ))
    ...
    for b in _walk_blocks(blocks):
        ...
        if b.type == BLOCK_WHILE:
            expr = (b.params.get("expr") or "").strip().lower()
            if expr in ("1", "true", "1==1"):
                issues.append(ValidationIssue(
                    "warn",
                    "This WHILE condition looks infinite (while(1)/true). Add a break/return/next/last, or set a max-iterations safety guard.",
                    b.label,
                    block_ref=b
                ))
        if b.type == BLOCK_FOR:
            max_iter = int(b.params.get("max_iterations") or 0)
            if max_iter <= 0:
                def _int_or_none(x):
                    if isinstance(x, int):
                        return x
                ...
                s = _int_or_none(b.params.get("start"))
                e = _int_or_none(b.params.get("end"))
                step = _step_or_none(b.params.get("inc_expr", "++"))
                op = (b.params.get("cmp_op") or "<=").strip()

                if s is not None and e is not None and step is not None:
                    likely_infinite = False
                    if op in ("<", "<=") and step <= 0:
                        likely_infinite = True
                    ...
                    if likely_infinite:
                        issues.append(ValidationIssue(
                            "warn",
                            "FOR loop looks non-terminating (increment moves the wrong direction). Change increment/comparison or set a Max Iterations safety guard.",
                            b.label,
                            block_ref=b
                        ))
        ...
        # -------------------------
    # 6) Whole-script lint removed
    # -------------------------
    # NOTE: Whole-script lint was intentionally removed to keep validation instant.
    # Perl generation now happens only in the (debounced) live preview.
...
class BlockStyleDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if style_key:
            ...
            # Muted group colors (NOT rainbow).
            group_rgb = {
            ...
            # If selected, let Qt's selection do most of the work
            selected = bool(option.state & QtWidgets.QStyle.StateFlag.State_Selected)

            # Stronger, theme-aware visibility
            fill = QtGui.QColor(group_rgb)
            border = QtGui.QColor(group_rgb)
            stripe = QtGui.QColor(group_rgb)

            if is_dark:
                fill.setAlpha(110 if not selected else 60)
                border.setAlpha(190 if not selected else 120)
                stripe.setAlpha(230 if not selected else 160)
            else:
                fill.setAlpha(70 if not selected else 40)
                border.setAlpha(140 if not selected else 90)
                stripe.setAlpha(210 if not selected else 140)

            # Inset so selection highlight still looks clean
            ...
            # --- left accent stripe ---
            stripe_w = 4
            sr = QtCore.QRect(r.left(), r.top(), stripe_w, r.height())
            painter.setPen(QtCore.Qt.PenStyle.NoPen)
            painter.setBrush(QtGui.QBrush(stripe))
            # Slightly smaller radius on the stripe so it looks crisp
            painter.drawRoundedRect(sr.adjusted(0, 0, 2, 0), 7, 7)
...
class ScriptTree(QtWidgets.QTreeWidget):
    ...
    def clear_issue_markers(self):
        """
        Remove any error/warn icons + foreground coloring from a previous validation pass.
        """
        def visit(item: QtWidgets.QTreeWidgetItem):
            # restore base icon
            self._restore_base_visuals_only(item)

            # reset text color back to default/palette
    ...
    def apply_issue_markers(self, issues: list[ValidationIssue]):
        ...
        for it in issues:
            ...
            bid = getattr(b, "uid", None) or id(b)
            prev = severity_by_block_id.get(bid)
            if prev is None or SEVERITY_ORDER[it.level] > SEVERITY_ORDER.get(prev, 0):
                severity_by_block_id[bid] = it.level

        # ✅ self is valid HERE
        icon_err = self.style().standardIcon(
            QtWidgets.QStyle.StandardPixmap.SP_MessageBoxCritical
        )
        icon_warn = self.style().standardIcon(
            QtWidgets.QStyle.StandardPixmap.SP_MessageBoxWarning
        )

        def visit(item: QtWidgets.QTreeWidgetItem):
            block = item.data(0, QtCore.Qt.ItemDataRole.UserRole)
            sev = severity_by_block_id.get(getattr(block, "uid", None)) if block else None

            if sev == "error":
                item.setIcon(0, icon_err)
                item.setForeground(0, QtGui.QBrush(QtGui.QColor("#ea4335")))
            elif sev == "warn":
                item.setIcon(0, icon_warn)
...
class BlockPropertyEditor(QtWidgets.QWidget):
    ...
    def _set_for_param(self, block: Block, key: str, value: str):
    ...
    def _build_while_form(self, block: Block):
        ...
        expr_edit.textChanged.connect(on_expr_change)
        guard = QtWidgets.QSpinBox()
        guard.setMinimum(0)
        guard.setMaximum(1_000_000)
        guard.setValue(int(block.params.get("max_iterations") or 0))
        guard.setToolTip("0 = no guard. If > 0, generator adds a last-if counter to prevent infinite loops.")
        self.add_labeled_widget("Max iterations safety guard (0 = off)", guard)

        def on_guard_change(val: int):
            block.params["max_iterations"] = int(val)
            self.block_changed.emit(block)

        guard.valueChanged.connect(on_guard_change)
    ...
    def _build_for_form(self, block: Block):
        ...
        cmp_combo.addItems(["<", "<=", ">", ">="])
        idx = cmp_combo.findText(str(cmp_op_val))
        if idx >= 0:
            cmp_combo.setCurrentIndex(idx)

        inc_edit = QtWidgets.QLineEdit(str(inc_expr_val))

        # --- NEW: exact for(...) live preview line
        preview = QtWidgets.QLineEdit()
        preview.setReadOnly(True)
        preview.setToolTip("Live preview of the generated for (...) header line.")
        ...
        self.add_labeled_widget("for (...) preview", preview)

        # --- NEW: safety guard (like WHILE)
        guard = QtWidgets.QSpinBox()
        guard.setMinimum(0)
        guard.setMaximum(1_000_000)
        guard.setValue(int(block.params.get("max_iterations") or 0))
        ...
        def _header_line() -> str:
            v = var_edit.text().strip() or "$i"
            s = start_edit.text().strip() or "0"
            e = end_edit.text().strip() or "10"
            op = cmp_combo.currentText().strip() or "<="
            inc = inc_edit.text().strip() or "++"

            # Ensure inc has a space when needed (+= 2 / -= 2), but not for ++/--
            if inc in ("++", "--"):
                inc_part = f"{v}{inc}"
            else:
                # If user typed "+=2" keep it; if they typed "+= 2" keep it
                inc_part = f"{v}{inc if inc.startswith(('+','-')) else (' ' + inc)}"
                # Most users type "+= 2" / "-= 1" which already includes space;
                # concatenation is fine either way.

                # If they typed "+= 2" (starts with +=), we want "$i+= 2"
                # That’s valid Perl; same for "-= 2".

            return f"for (my {v} = {s}; {v} {op} {e}; {inc_part})"

        def update():
            # Persist params
            block.params["var_name"] = var_edit.text().strip() or "$i"

            s_txt = start_edit.text().strip()
            e_txt = end_edit.text().strip()

            # keep ints as ints when possible (nice for JSON + display), else keep raw
            def _int_or_str(t: str):
                t2 = t.replace("_", "").strip()
                return int(t2) if t2.isdigit() else t

            block.params["start"] = _int_or_str(s_txt) if s_txt else 0
            block.params["end"] = _int_or_str(e_txt) if e_txt else 10
            block.params["cmp_op"] = cmp_combo.currentText().strip() or "<="

            inc = inc_edit.text().strip()
            block.params["inc_expr"] = inc if inc else "++"

            # Update block label + preview line
            line = _header_line()
            preview.setText(line)

            # Keep the ScriptTree label readable
            block.label = line
        ...
        # Wire up signals
        var_edit.textChanged.connect(lambda _=None: update())
        start_edit.textChanged.connect(lambda _=None: update())
        end_edit.textChanged.connect(lambda _=None: update())
        cmp_combo.currentIndexChanged.connect(lambda _=None: update())
        inc_edit.textChanged.connect(lambda _=None: update())
        guard.valueChanged.connect(on_guard_change)

        # Initialize preview/label
        update()
...
def generate_perl(blocks: List[Block], plugins: PluginRegistry, npc_id: Optional[int] = None) -> str:
    lines: List[str] = []
    lines.append("# Generated by EQEmu Script Builder v1.22")
    ...
    def render_block(block: Block, indent: int):
        ...
        if t == BLOCK_EVENT:
            ...
            expr = block.params.get("expr", "1")
            max_iter = int(block.params.get("max_iterations") or 0)

            if max_iter > 0:
                guard_var = f"$__guard_{block.uid[:8]}"
                emit(f"my {guard_var} = 0;", indent)
                emit(f"while ({expr}) {{", indent)
                emit(f"last if ++{guard_var} > {max_iter};  # safety guard", indent + 1)
                for child in block.children:
                    render_block(child, indent + 1)
                emit("}", indent)
            else:
            ...
            inc_expr = block.params.get("inc_expr") or "++"

            start_s = str(start)
            end_s   = str(end)

            max_iter = int(block.params.get("max_iterations") or 0)
            guard_var = f"$__for_guard_{block.uid[:8]}"

            # Build the increment piece
            if inc_expr.strip() in ("++", "--"):
                inc_part = f"{var_name}{inc_expr.strip()}"
            else:
                inc_part = f"{var_name}{inc_expr.strip()}"
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EQEmu Script Builder v1.22")
        ...
        # OUTER splitter: palette (left) | center stack (top work area + bottom diagnostics)
        outer = QtWidgets.QSplitter(QtCore.Qt.Orientation.Horizontal)
        outer.addWidget(self.palette_tabs)  # left

        # Center stack: top = script+props, bottom = diagnostics
        self.center_splitter = QtWidgets.QSplitter(QtCore.Qt.Orientation.Vertical)
        self.center_splitter.addWidget(self.right_splitter)  # top
        self.center_splitter.addWidget(self.diagnostics)     # bottom

        # Make diagnostics about bottom quarter by default
        self.center_splitter.setStretchFactor(0, 4)  # work area
        self.center_splitter.setStretchFactor(1, 1)  # diagnostics
        self.center_splitter.setSizes([900, 300])    # tweak if you want

        outer.addWidget(self.center_splitter)        # center (stacked)
        ...
        outer.setCollapsible(1, False)  # center stack

        # Let the user collapse diagnostics inside the vertical center splitter
        self.center_splitter.setCollapsible(0, False)  # top work area
        self.center_splitter.setCollapsible(1, True)   # bottom diagnostics

        outer.setStretchFactor(0, 1)  # palette
        outer.setStretchFactor(1, 6)  # center stack
        outer.setSizes([260, 1470])   # tweak to taste

        self.setCentralWidget(outer)
        QtCore.QTimer.singleShot(0, lambda: self.center_splitter.setSizes([750, 250]))
        ...
        # ---- Zoom controls (Ctrl +/- / Ctrl+wheel) ----
        self._install_zoom_controls()
        
        self._perl_preview_timer = QtCore.QTimer(self)
        self._perl_preview_timer.setSingleShot(True)
        self._perl_preview_timer.timeout.connect(self._update_perl_preview)
    ...
    # ------------------------------
    def _install_zoom_controls(self):
        # Track per-widget base sizes using QObject properties (no fragile id(block) stuff)
        self._zoom_min_pt = 7
        self._zoom_max_pt = 22

        # Key shortcuts (work regardless of which child widget has focus)
        def sc(seq, fn):
            s = QtGui.QShortcut(QtGui.QKeySequence(seq), self)
            s.setContext(QtCore.Qt.ShortcutContext.ApplicationShortcut)
            s.activated.connect(fn)
            return s

        # Ctrl + Plus / Equals
        self._zoom_sc_plus1 = sc("Ctrl+=", lambda: self._zoom_active(+1))
        self._zoom_sc_plus2 = sc("Ctrl++", lambda: self._zoom_active(+1))

        # Ctrl + Minus
        self._zoom_sc_minus = sc("Ctrl+-", lambda: self._zoom_active(-1))

        # Ctrl + 0 (reset)
        self._zoom_sc_reset = sc("Ctrl+0", self._zoom_reset_active)

        # Ctrl + Mouse Wheel (global)
        app = QtWidgets.QApplication.instance()
        if app is not None:
            app.installEventFilter(self)

    def eventFilter(self, obj, event):
        # Ctrl + wheel zoom for the widget under mouse/focus
        if event.type() == QtCore.QEvent.Type.Wheel:
            mods = event.modifiers()
            if mods & QtCore.Qt.KeyboardModifier.ControlModifier:
                # Typical mouse wheels: angleDelta().y() is +/-120 per notch
                dy = event.angleDelta().y()
                if dy != 0:
                    step = 1 if dy > 0 else -1
                    self._zoom_active(step, prefer_under_mouse=True)
                return True  # consume event so it doesn't also scroll
        return super().eventFilter(obj, event)

    def _zoom_active(self, delta_steps: int, prefer_under_mouse: bool = False):
        w = self._zoom_target_widget(prefer_under_mouse=prefer_under_mouse)
        if w is None:
            return
        self._apply_font_delta(w, delta_steps)

    def _zoom_reset_active(self):
        w = self._zoom_target_widget(prefer_under_mouse=True)
        if w is None:
            return

        base = w.property("_zoom_base_pt")
        if isinstance(base, int) and base > 0:
            f = w.font()
            f.setPointSize(base)
            w.setFont(f)

            # QTreeWidget/QListWidget sometimes need a repaint to “feel” immediate
            w.update()

    def _zoom_target_widget(self, prefer_under_mouse: bool = False):
        app = QtWidgets.QApplication.instance()
        if app is None:
            return None

        # Prefer widget under mouse for wheel zoom (feels natural)
        if prefer_under_mouse:
        ...
        # If focus is on a viewport/child, walk up to a useful parent widget
        supported = (
    ...
    def _apply_font_delta(self, widget: QtWidgets.QWidget, delta_steps: int):
        # If it's a text editor with built-in zoomIn/zoomOut, use that
        # (keeps cursor/selection behavior nice)
        if hasattr(widget, "zoomIn") and hasattr(widget, "zoomOut"):
            # store base size once
            self._ensure_zoom_base(widget)
            if delta_steps > 0:
                widget.zoomIn(delta_steps)
            else:
                widget.zoomOut(-delta_steps)
            return
        ...
        pt = max(self._zoom_min_pt, min(self._zoom_max_pt, pt + delta_steps))
        f.setPointSize(pt)
        widget.setFont(f)
        widget.update()

    def _ensure_zoom_base(self, widget: QtWidgets.QWidget):
        base = widget.property("_zoom_base_pt")
        if isinstance(base, int) and base > 0:
            return
        f = widget.font()
        pt = f.pointSize()
        if pt <= 0:
            pt = 10
        widget.setProperty("_zoom_base_pt", int(pt))
    
    def _ensure_perl_preview_timer(self):
        if getattr(self, "_perl_preview_timer", None):
            return
        self._perl_preview_timer = QtCore.QTimer(self)
        self._perl_preview_timer.setSingleShot(True)
        self._perl_preview_timer.timeout.connect(self._update_perl_preview)
        self._last_blocks_for_preview = []

    def _update_perl_preview(self):
        blocks = getattr(self, "_last_blocks_for_preview", None) or []

        gen = globals().get("generate_perl")
        if not callable(gen):
            self.diagnostics.set_perl("# Live preview error:\n# generate_perl() is not defined.\n")
            return

        try:
            perl = gen(blocks, self.registry)
        except Exception as e:
    ...
    def toggle_diagnostics(self):
        """
        Collapse / restore diagnostics pane (now bottom of center splitter).
        """
        ...
        # Expecting: [work_area, diagnostics]
        if len(sizes) != 2:
            return
    ...
    def refresh_diagnostics(self):
        ...
        # Debounced Perl preview (validation stays instant)
        self._ensure_perl_preview_timer()
        self._last_blocks_for_preview = blocks
        self._perl_preview_timer.start(120)  # tweak: 80–200ms feels good
